...
func _wave_load(wave_number: int) -> Dictionary:
	if boss_rush_mode:
		return wave_library.call("boss_rush_wave_data", wave_number) as Dictionary
	if daily_seed_mode:
		return wave_library.call("daily_seed_wave_data", wave_number) as Dictionary
	if draft_defense_mode:
		return wave_library.call("draft_defense_wave_data", wave_number, _draft_package_title(), MAX_WAVES) as Dictionary
	if endless_mode:
		return wave_library.call("endless_wave_data", wave_number) as Dictionary
	return wave_library.call("load_wave", wave_number) as Dictionary


func _daily_seed_label() -> String:
	return str(wave_library.call("daily_seed_label"))
